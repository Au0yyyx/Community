...
local function item(m)
 ...
 local ring=Instance.new("Part");ring.Name="OM_HitRange";ring.Anchored=true;ring.CanCollide=false;ring.CanQuery=false;ring.CanTouch=false;ring.Shape=Enum.PartType.Cylinder;ring.Material=Enum.Material.Neon;ring.Transparency=.7;ring.Color=Color3.fromRGB(255,80,80);ring.Parent=workspace
...
App.Connections[#App.Connections+1]=RunService.RenderStepped:Connect(function()
 local pf=playersFolder();if pf then for _,m in ipairs(pf:GetChildren())do local r=root(m);if m~=LP.Character and r then local v=item(m);local team=m:GetAttribute("Team");local col=team=="EXE" and Color3.fromRGB(255,65,65) or Color3.fromRGB(65,180,255);v.H.FillColor=col;v.H.OutlineColor=Color3.new(1,1,1);v.H.Enabled=App.Settings.ESP;v.B.Enabled=App.Settings.ESP and App.Settings.Names;v.T.Text=string.format("%s [%s]",m.Name,tostring(m:GetAttribute("State") or team));v.R.Color=col;v.R.Size=Vector3.new(.15,App.Settings.HitRadius*2,App.Settings.HitRadius*2);v.R.CFrame=CFrame.new(r.Position-Vector3.new(0,2.7,0))*CFrame.Angles(0,0,math.rad(90));v.R.Transparency=App.Settings.Hitboxes and .7 or 1 end end end
...
local e=Tabs.ESP:AddLeftGroupbox("ESP");e:AddToggle("OMESP",{Text="Player ESP",Default=false,Callback=function(v)App.Settings.ESP=v end});e:AddToggle("OMNames",{Text="Names and state",Default=true,Callback=function(v)App.Settings.Names=v end});e:AddToggle("OMHitboxes",{Text="Visual circular hitboxes",Default=false,Callback=function(v)App.Settings.Hitboxes=v end});e:AddSlider("OMHitRadius",{Text="Hitbox radius",Default=8,Min=1,Max=30,Rounding=2,Suffix=" studs",Callback=function(v)App.Settings.HitRadius=v end})
