...
local controls=panel(2,82);label(controls,"LIVE SETTINGS",UDim2.fromOffset(10,4),UDim2.new(1,-150,0,18),Enum.Font.GothamBold,13,Color3.fromRGB(245,233,255))
local shopView="both";local viewButton=Instance.new("TextButton");viewButton.Position=UDim2.new(1,-126,0,3);viewButton.Size=UDim2.fromOffset(116,22);viewButton.BackgroundColor3=Color3.fromRGB(17,17,17);viewButton.BorderSizePixel=0;viewButton.Text="SHOP VIEW: BOTH";viewButton.TextColor3=Color3.fromRGB(216,180,254);viewButton.Font=Enum.Font.GothamBold;viewButton.TextSize=9;viewButton.Parent=controls;Instance.new("UICorner",viewButton).CornerRadius=UDim.new(0,7);local vst=Instance.new("UIStroke",viewButton);vst.Color=Color3.fromRGB(126,34,206);viewButton.MouseButton1Click:Connect(function()shopView=shopView=="both"and"current"or(shopView=="current"and"next"or"both");viewButton.Text="SHOP VIEW: "..shopView:upper()end)
...
local ownedP,ownedG=section(94,174,"Owned Upgrades");local shopP,shopG,shopMoney=section(278,174,"Current Shop");local nextP,nextG,nextMoney=section(462,174,"Next Shop");local altarP,altarG,altarMoney=section(646,174,"Protection Altars");local purifyP,purifyG,purifyMoney=section(830,174,"Purification Costs")
local best=panel(1014,56);scroll.CanvasSize=UDim2.fromOffset(0,1080);local bestA=label(best,"BEST OPTION",UDim2.fromOffset(10,5),UDim2.new(1,-20,0,20),Enum.Font.GothamBold,11,Color3.fromRGB(216,180,254));local bestB=label(best,"Calculating...",UDim2.fromOffset(10,25),UDim2.new(1,-20,0,22),Enum.Font.GothamMedium,10)
...
local last="";task.spawn(function()while not App.Destroyed do local s,choices,basket,spent=calculate();lv.Text=tostring(s.level);pc.Text=tostring(s.players);pt.Text=s.party:upper();df.Text=s.difficulty:upper()..(s.nothing and" / NOTHING?"or"")
 ...
 local remain=s.money-selectedTotal;shopMoney.Text="Level "..s.level.." • Golden Gifts left: "..remain;nextMoney.Text="Level "..(s.level+1).." • Golden Gifts left: "..remain;altarMoney.Text="Golden Gifts left: "..remain;purifyMoney.Text=altarMoney.Text;altarP.BackgroundTransparency=s.level>=8 and .08 or .55;purifyP.BackgroundTransparency=s.level>=14 and .08 or .55
 local y=278;shopP.Visible=shopView~="next";nextP.Visible=shopView~="current";if shopP.Visible then shopP.Position=UDim2.fromOffset(6,y);y+=184 end;if nextP.Visible then nextP.Position=UDim2.fromOffset(6,y);y+=184 end;altarP.Position=UDim2.fromOffset(6,y);y+=184;purifyP.Position=UDim2.fromOffset(6,y);y+=184;best.Position=UDim2.fromOffset(6,y);scroll.CanvasSize=UDim2.fromOffset(0,y+66)
